...
void genrateplot_resovsE_UL(string legendname,string plot_name, bool xaxis_pt, bool corrE=false,   bool raw=true)
{
...
//=========  (Mon Apr  1 19:35:54 2019) by ROOT version 6.14/06
  ...
  else  graph2 = (TGraph*) inputfile2 -> Get("Graph;2");
  ...
  if(corrE && !raw){
    sprintf(path,"plots/corrE_%s_reso.gif",plot_name.c_str());
    sprintf(path1,"plots/corrE_%s_reso.pdf",plot_name.c_str());
    sprintf(path2,"plots/corrE_%s_reso.png",plot_name.c_str());
  }
  else if(!corrE && raw){
    sprintf(path,"plots/raw_%s_reso.gif",plot_name.c_str());
    sprintf(path1,"plots/raw_%s_reso.pdf",plot_name.c_str());
    sprintf(path2,"plots/raw_%s_reso.png",plot_name.c_str());
  }  
  else if(!raw && !corrE){
    sprintf(path,"plots/corrEtawrtraw_%s_reso.gif",plot_name.c_str());
    sprintf(path1,"plots/corrEtawrtraw_%s_reso.pdf",plot_name.c_str());
    sprintf(path2,"plots/corrEtawrtraw_%s_reso.png",plot_name.c_str());
  }
  else if(raw && corrE){
    sprintf(path,"plots/corrEwrtraw_%s_reso.gif",plot_name.c_str());
    sprintf(path1,"plots/corrEwrtraw_%s_reso.pdf",plot_name.c_str());
    sprintf(path2,"plots/corrEwrtraw_%s_reso.png",plot_name.c_str());
  }

  if(xaxis_pt && !raw && !corrE){ 
    sprintf(path_root1,"UL2016/response");
    sprintf(path_root2,"UL2017/response");
    sprintf(path_root3,"UL2018/response");
    sprintf(rootfile1,"correta");
    ...
    graph1 = (TGraph*) inputfile1 -> Get("Graph;2");
    graph2 = (TGraph*) inputfile2 -> Get("Graph;2");
    graph3 = (TGraph*) inputfile3 -> Get("Graph;2");
    respHisto__1 = (TH2F*) inputfile1 -> Get("resoHisto");
    sprintf(rootfile1,"Raw");
    sprintf(rootfile2,"Raw");
    sprintf(rootfile3,"Raw");
    ...
    sprintf(path,"plots/corrEtawrtraw_%s_reso_pt.gif",plot_name.c_str());
    sprintf(path1,"plots/corrEtawrtraw_%s_reso_pt.pdf",plot_name.c_str());
    sprintf(path2,"plots/corrEtawrtraw_%s_reso_pt.png",plot_name.c_str());
  } 
  ...
  cout<<"file name --> "<<plot_name<<endl;  
   ...
   TGraph *graph1_,*graph2_;
   if(xaxis_pt) {
     graph1_ = (TGraph*) inputfile1_ -> Get("resolution;1");
     graph2_ = (TGraph*) inputfile2_ -> Get("resolution;1");
   }
   else {
     graph1_ = (TGraph*) inputfile1_ -> Get("Graph;2");
     graph2_ = (TGraph*) inputfile2_ -> Get("Graph;2");
   }
  ...
  //  if(isPaper) textOnTop.DrawLatexNDC(0.12,0.91,"CMS #it{#bf{Simulation Supplementary}}")
   ...
   TLegend* legends = new TLegend(0.12, 0.7, 0.9, 0.9,"","brNDC"); // the numbers determine the position of the box                                                                          
   legends->SetFillColor(0);
   legends->SetHeader(legendname.c_str(),"C");
   if((!raw && !corrE) || (xaxis_pt && !raw && !corrE)){
     ...
     legends->AddEntry(graph1,"2016 (Before calibration)","P");
     legends->AddEntry(graph2,"2017 (Before calibration)","P");
     legends->AddEntry(graph3,"2018 (Before calibration)","P");
   }
   else{
